---
title: Who Touched My Packages?
description: A beautiful CLI tool for auditing dependencies and finding vulnerabilities
template: splash
hero:
  tagline: Secure your dependencies with style 🛡️<br/>Powered by Threat Point
  actions:
    - text: Get Started
      link: /who-touched-my-packages/getting-started/installation/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/Point-Wild/who-touched-my-packages
      icon: external
      variant: minimal
---

import { Card, CardGrid } from '@astrojs/starlight/components';

## Features

<CardGrid stagger>
  <Card title="Multi-Ecosystem" icon="puzzle">
    Supports npm, Python, Ruby, Go, and Rust with more coming soon
  </Card>
  <Card title="Remote Repository Scanning" icon="github">
    Clone and scan any Git repository directly without manual setup
  </Card>
  <Card title="Multiple Data Sources" icon="random">
    Queries OSV for comprehensive vulnerability coverage
  </Card>
  <Card title="Provenance Verification" icon="approve-check">
    Automatically checks for SLSA provenance attestations to verify package integrity in NPM and PyPI
  </Card>
  <Card title="Beautiful UI" icon="star">
    Colorful, emoji-rich terminal output with automatic light/dark mode detection
  </Card>
  <Card title="CI/CD Ready" icon="star">
    JSON output and exit codes make it perfect for automation pipelines
  </Card>
  <Card title="Severity Filtering" icon="information">
    Filter vulnerabilities by severity level (CRITICAL, HIGH, MEDIUM, LOW)
  </Card>
  <Card title="Dependency Pinning" icon="download">
    Ensure dependencies are pinned for Node.js and Python
  </Card>
  <Card title="Recursive Scanning" icon="document">
    Automatically finds all dependency files in your project tree
  </Card>
  <Card title="Fast & Efficient" icon="rocket">
    Parallel API requests and smart caching for quick scans
  </Card>
  <Card title="Extensible" icon="seti:config">
    Easy to add new data sources and package managers
  </Card>
  <Card title="Optional LLM Scanning" icon="magnifier">
    Optionally provide an API key to scan with frontier LLM models
  </Card>
</CardGrid>

## Quick Start

Install globally:

```bash
npm install -g who-touched-my-packages
```

Scan your project:

```bash
wtmp
```

That's it! The tool will recursively scan your project and report any vulnerabilities 🎉

## Example Output

```
🛡️ Who Touched My Packages?
  Scanning dependencies for vulnerabilities...

✔ Found 2 dependency file(s)
✔ Parsed 16 package(s)

════════════════════════════════════════════════════════════
🛡️ Security Audit Summary
════════════════════════════════════════════════════════════

Scanned Packages: 16
Total Vulnerabilities: 3

🔴 Critical: 1
🟠 High: 2

════════════════════════════════════════════════════════════
```

## Why Who Touched My Packages?

- **Beautiful UX**: Security tools should be pleasant to use
- **Multiple Sources**: Don't rely on a single vulnerability database
- **Extensible**: Easy to add new data sources and package managers
- **Fast**: Optimized for large monorepos
- **Free**: No API keys or paid plans required

## Next Steps

<CardGrid>
  <Card title="Installation" icon="rocket">
    [Install the tool](/who-touched-my-packages/getting-started/installation/) and run your first scan
  </Card>
  <Card title="Quick Start" icon="open-book">
    Learn the [basics](/who-touched-my-packages/quick-start/) and scan your first project
  </Card>
  <Card title="Provenance Verification" icon="approve-check">
    Understand [how provenance checking works](/who-touched-my-packages/features/provenance/) and why it matters
  </Card>
</CardGrid>
