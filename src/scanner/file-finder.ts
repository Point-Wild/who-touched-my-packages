...
const TARGET_FILES = new Set(['package.json', 'requirements.txt', 'Cargo.toml', 'Cargo.lock']);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const type = entry.name as 'package.json' | 'requirements.txt' | 'Cargo.toml' | 'Cargo.lock';
          results.push({
            path: fullPath,
            type,
